...
```{r}
full_model = lm(total_UPDRS ~ . - motor_UPDRS , data = park_trn_data)
fit_fow = lm(total_UPDRS ~ 1, data = park_trn_data)
...
```{r}
new_model = lm( total_UPDRS ~ . ^ 2, data = park_trn_data)

fit_back_aic = step(new_model, direction = "backward", trace = 0)
